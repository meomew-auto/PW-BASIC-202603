...
test.describe("☕ [CASE 10] Enterprise Hybrid Super E2E & Multi-Tab Live Workflow", () => {
  ...
  test("01 - [SUPER HYBRID E2E] Fast API Preparation ➔ Live Admin Products UI Audit ➔ Table Helpers Scan ➔ API Zod Contract Audit", async ({
    ...
    let productApi: ProductApiClient;
    let apiContext: any;

    await test.step("⚡ Pha 1: API Fast Data Retrieval (AOM Client)", async () => {
      ...
      productApi = new ProductApiClient(apiContext, workerStaffSnapshot.token);
    ...
    // ══════════════════════════════════════════════════════════════════════════
    await test.step("🖥️ Pha 2: Live UI Admin Table Navigate & Audit (TableColumnHelpers)", async () => {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    await test.step("📊 Pha 3: Quét và Check toàn bộ bảng sản phẩm qua TableColumnHelpers", async () => {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    await test.step("🩺 Pha 4: API Database Deep Audit qua Zod Schema Contract Assert", async () => {
  ...
  test("02 - [LIVE MULTI-TAB WORKFLOW] Admin Order Detail #103 ➔ In Hóa Đơn (New Tab) ➔ Đối soát mã đơn đồng nhất", async ({
    ...
    // ══════════════════════════════════════════════════════════════════════════
    await test.step("📍 Bước 1: Navigate mở trang Chi tiết đơn hàng #103 trên hệ thống live", async () => {
    ...
    await test.step("🌟 Bước 2: Click 'In hóa đơn' và đón bắt tab mới (context.waitForEvent)", async () => {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    await test.step("🧾 Bước 3: Đối soát Assert mã đơn hàng trên tab Hóa đơn qua POM NekoInvoicePage", async () => {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    await test.step("❌ Bước 4: Đóng Tab Hóa đơn và Check an toàn hoàn trả tiêu điểm về tab gốc", async () => {
