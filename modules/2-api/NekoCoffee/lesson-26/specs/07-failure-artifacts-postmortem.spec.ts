...
test.describe("🚨 [CASE 07] Failure Post-Mortem & Artifact Rescue", () => {
  test("01 - [ARTIFACT RESCUE] Thu thập ảnh chụp màn hình và video sự cố", async ({
    ...
    let isSimulateFailure: boolean;

    await test.step("1. [RENDER UI] Nạp giao diện màn hình thu thập bằng chứng sự cố", async () => {
      isSimulateFailure = process.env.SIMULATE_FAILURE === "true";
    ...
    await test.step("2. [ASSERT & POST-MORTEM] Kiểm chứng nút đơn hàng hoặc kích hoạt cứu hộ Artifacts", async () => {
